// 3166 Final Project (Player 2) by Brian Stanwood
...
public class PlayerTwo {
	public static void main(String[] args) throws UnknownHostException, IOException {
		...
		Socket s = new Socket("127.0.0.1",2345);   // create socket for communication with local IP and a specified port number
		DataInputStream din = new DataInputStream(s.getInputStream());  
		DataOutputStream dout = new DataOutputStream(s.getOutputStream());
		
		// test player 2/server connection
		...
		System.out.println("Waiting for player 1's turn to be over...");
		...
		while (gameLoop != 0) {   // Game loop begins
			...
			//while (checkX < 2) {  // turn loop begins
			    ...
			    for(int xloop = 0; xloop < 2; xloop++) { // runs twice to get both coordinates (xy, xy)
			    	...
			    	s = new Socket("127.0.0.1",1234);               // takes in the current playboard and prints
			        ...
			        while (xIsValid == 0) {   
			        	...
			        	s = new Socket("127.0.0.1",2345);               // sends out xy value 
						dout = new DataOutputStream(s.getOutputStream());
						din = new DataInputStream(s.getInputStream());
